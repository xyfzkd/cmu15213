 ...
 *****************************************************************/
/* get the alloc of the block from header element */
#define get_a_alloc(p) (get(p) & 0x1)
/* get the alloc of the previous block from header element */
#define get_b_alloc(p) ((get(p) & 0x2)>>1)

#define pack_nb(size, b_alloc, a_alloc) ((size)|((b_alloc)<<1)|(a_alloc))

#define tag_nb(p,b_alloc,a_alloc) (put((p),pack_nb(get_size(p),(b_alloc),(a_alloc))))
...
static void *coalesce_nb(void *bp);
 ...
 */
static void *coalesce_nb(void *bp){
    size_t prev_alloc = get_b_alloc(h_bp(bp));
    size_t next_alloc = get_a_alloc(h_bp(next_block(bp)));
    size_t size = get_size(h_bp(bp));

    if (prev_alloc && next_alloc) {
        tag_nb(h_bp(bp),1,0);
        tag_nb(f_bp(bp),1,0);
        tag_nb(h_bp(next_block(bp)),0,1);
        return bp;
    }
    else if (prev_alloc && !next_alloc){
        size += get_size(h_bp(next_block(bp)));
        put(h_bp(bp),pack_nb(size,1,0));
        put(f_bp(bp),pack_nb(size,1,0));
    }
    else if (!prev_alloc && next_alloc){
        int prev_b_alloc = get_b_alloc(f_bp(prev_block(bp)));
        size += get_size(f_bp(prev_block(bp)));
        put(h_bp(prev_block(bp)),pack_nb(size,prev_b_alloc,0));
        put(f_bp(prev_block(bp)),pack_nb(size,prev_b_alloc,0));
        tag_nb(h_bp(next_block(bp)),0,1);
        bp = prev_block(bp);
    }
    else{
